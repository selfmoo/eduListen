...
type
  ...
  TSynBigTableCustomHeader = (sbtGetMagic, sbtRead, sbtWrite,
    sbtAfterRead);
...
{$endif}
    ...
    // - if Action is sbtGetMagic, should return a magic integer value, which
    // will be used to identify the file format
...
{$endif}
    ...
    // - return the unique ID created to identify this data
    // - returns 0 on adding error (e.g. if a tftUnique constraint failed)
    function RecordAdd(const aRecord: TSynTableData): integer;
    /// retrieve a record as a TSynTableData to access its properties
    // - using TSynTableData is faster than a TSynTableVariantType variant
    // - this overloaded function doesn't use a function return, therefore
    // will avoid a Record copy content (faster)
...
begin
  if Action=sbtGetMagic then
    // TSynBigTable magic number for file content identification
    result := integer($ABAB0006) else
    result := 0;
end;
...
begin
  ...
  if (magic<>CustomHeader(sbtGetMagic,nil)) or (CustomHeaderSize<=0) then
...
{$endif}
    ...
    try
      ...
      magic := CustomHeader(sbtGetMagic,nil);
...
function TSynBigTableRecord.RecordAdd(const aRecord: TSynTableData): integer;
begin
  if aRecord.SBF='' then
    result := 0 else
    result := Add(aRecord.SBF);
end;
...
begin
  ...
  for i := 0 to high(Deleted) do
    if Check(not T.Get(Deleted[i],Data)) then
      exit;
  if Pack then begin
    for i := 1 to n do
      if IntegerScanIndex(pointer(Deleted),length(Deleted),i)<0 then
      if T.Get(i,Data) then 
        if Check(TestString(i,Data)) then
          exit else
        if withString then
        if Check(TS.Get(Int32ToUTF8(i),Data)) or Check(TestString(i,Data)) then
          exit;
  end else
    for i := 1 to n do 
      if T.Get(i,Data) THEN
  ...
  for i := 1 to n do begin
    St := T.GetAsStream(i);
    if (St<>nil) and Check(TestStream(i,St)) then
      exit;
  end;
  tot := 0;
  T.GetIterating(T.TestIterateEvent,ioID,@tot);
...
{$ifndef LVCL}
  for i := 1 to n do begin
    vari := T.VariantGet(i);
    if Check(vari.text=By8[n-i]) or Check(vari.ID=i) or
       Check(vari.Int=i-1,'Read as Variant') then exit;
  end;
  for i := 1 to n do begin
    T.RecordGet(i,rec);
    if Check(RawUTF8(rec.GetFieldValue(fText))=By8[n-i]) or
       Check(integer(rec.GetFieldValue(fInt))=i-1,'Read as TSynTableData') then
      exit;
  end;
{$endif}
  for i := 1 to n do begin
    if DoRecord then
      p := TRec.GetPointer(i,s) else
      p := TMeta.GetMetaDataFromID(i);
    if Check(fInt.GetInteger(p)=i-1) or
       Check(fText.GetRawUTF8(p)=By8[n-i],'Read direct') then
      exit;
  end;
  IDCount := 0; // not mandatory (if ID=nil, T.Search will do it)
  for i := n-50 to n-1 do
    if Check(T.Search(fText,fText.SBF(By8[n-i]),ID,IDCount,0,ioPhysical)) or
       Check(IDCount=1) or Check(ID[0]=i,'Search 50 Text iterating') then
        exit else
        IDCount := 0;
  for i := n-200 to n-1 do
    if Check(T.Search(fText,fText.SBF(By8[n-i]),ID,IDCount)) or
      Check(IDCount=1) or Check(ID[0]=i,'Search 200 Text using index') then
        exit else
        IDCount := 0;
  for i := n-50 to n-1 do
    if Check(T.Search(fInt,fInt.SBF(i),ID,IDCount,0,ioPhysical)) or
       Check(IDCount=1) or Check(ID[0]=i+1,'Search 50 Int iterating') then
        exit else
        IDCount := 0;
  for i := n-200 to n-1 do
    if Check(T.Search(fInt,fInt.SBF(i),ID,IDCount)) or
      Check(IDCount=1) or Check(ID[0]=i+1,'Search 200 Int using index') then
        exit else
        IDCount := 0;
  if not DoRecord then
    for i := 1 to n do
      if Check(TMeta.Get(i,s)) or Check(TestString(i,s)) then
        exit;
  Result := true;
end;
var fText, fInt, fBool: TSynTableFieldProperties;
...
begin
  ...
  try
    ...
    if Check(TRTest) then exit;
    T.UpdateToFile;
    T.Free;
    if DoRecord then
      T := TSynBigTableRecord.Create(FN,'test') else
      T := TSynBigTableMetaData.Create(FN,'test');
    if not TRTest then exit;
    ...
    for i := 0 to (n shr 3)-1 do begin
      aID := i shl 3+1;
      rec := T.RecordGet(aID);
      if Check(rec.ID=aID) then exit;
      if Check(not fBool.GetBoolean(Pointer(rec.SBF))) then exit;
      rec.SetFieldSBFValue(fBool,fBool.SBF(true));
      if Check(T.RecordUpdate(rec)) then exit;
      rec := T.RecordGet(aID);
      if Check(fBool.GetBoolean(Pointer(rec.SBF))) then exit;
    end;
    T.Pack;
    if Check(TRTest) then exit;
    fInt := T.Table['int'];
    for i := 0 to nInt-1 do begin
      ...
      if Check(T.RecordUpdate(rec)) then exit;
      if Check(fInt.GetInteger(pointer(rec.SBF))=n*2+aID) then exit;
    end;
    for i := 0 to nInt-1 do begin
      if (i and 7=0) and (i<(nInt shl 3)) then
        Value := n*2+i+1 else
        Value := i;
      IDCount := 0;
      if Check(T.Search(fInt,fInt.SBF(Value),IDs,IDCount)) or
         Check(IDCount=1) or Check(IDs[0]=i+1) then
          exit;
    end;
    result := true;
  finally
...
begin
  ...
  try
    for i := 1 to n do
      if not Check(T.Add(CreateString(i))=i) then
      if T.CurrentInMemoryDataSize>10 shl 20 then // write on disk every 10 MB
        T.UpdateToFile;
    Check(T.Count=n);
    for i := 1 to n do
      if Check(T.Get(i,Data)) or Check(TestString(i,Data)) then
        exit;
    T.Free;
    T := TSynBigTable.Create(FN); // verify after read
    for i := 1 to n do begin
      id := Random(n)+1; // test random access speed
      if Check(T.Get(id,Data)) or Check(TestString(id,Data)) then
        exit;
    end;
    for i := 1 to n do
      if Check(T.Get(i,Data)) or Check(TestString(i,Data)) then
...
begin
  ...
  try
    ...
    n := Random(10)+500;
    for i := 1 to n do
      if Check(TS.Add(TTestBigTable.CreateString(i),Int32ToUTF8(i))=i) then
        exit else
      if TS.CurrentInMemoryDataSize>10 shl 20 then // write on disk every 10 MB
        TS.UpdateToFile;
    Check(TS.Count=n);
    for i := 1 to n do
      if Check(TS.Get(i,Data)) or Check(TestString(i,Data)) or
         Check(TS.Get(Int32ToUTF8(i),Data)) or Check(TestString(i,Data)) then
        exit;
    TS.Free;
    TS := TSynBigTableString.Create(FN); // verify after read
    for i := 1 to n do
    ...
    for i := 1 to n do begin
      id := Random(n)+1; // test random access
      if Check(TS.Get(id,Data)) or Check(TestString(id,Data)) or
         Check(TS.Get(Int32ToUTF8(id),Data)) or Check(TestString(id,Data)) then
        exit;
    end;
    for i := n+1 to n+100 do // create some fInMemory[] data before Pack
    ...
    for i := 1 to n do
      if Check(TS.Get(i,Data)) or Check(TestString(i,Data)) or
         Check(TS.Get(Int32ToUTF8(i),Data)) or Check(TestString(i,Data)) then
        exit;
    if Check(Delete(TSynBigTable(TS),n,true,true)) then
      exit;
    TS.Free;
    TS := TSynBigTableString.Create(FN); // verify after read
